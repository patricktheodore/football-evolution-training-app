  ...
  if (Auth.loggedIn()) {
    ...
    return (
        <Stack
          ...
          sx={{ pr: 2 }}
        >
          <Button variant="primary" href="/login" sx={{ fontSize: '1rem', fontWeight: 'bold'}}>Sign In</Button>
          <Button variant="primary" href="/register" sx={{ fontSize: '1rem', fontWeight: 'bold'}}>Register</Button>
        </Stack>
    );
  }
};
      ...
      <AppBar position="fixed" open={open} className="nav-bar" elevation={0}>
        <Toolbar sx={{ justifyContent: 'space-between' }} className="nav-bar">
          <IconButton
            ...
            edge="start"
            sx={{ mr: 2, ...(open && { display: 'none' }), color: 'black', ml: 3, }}
          >
      ...
      >
        <DrawerHeader>
          <IconButton onClick={handleDrawerClose}>
            {theme.direction === 'ltr' ? <ChevronLeftIcon fontSize='medium'/> : <ChevronRightIcon fontSize='medium' />}
          </IconButton>
        </DrawerHeader>
        <Divider />
        <MenuList dense>
          ...
          <MenuItem>
              <Link to="/aboutUs" className='menu-link' onClick={handleDrawerClose}>
                About Us
              </Link>
          </MenuItem>
          <MenuItem>
              <Link to="/trainWithUs" className='menu-link' onClick={handleDrawerClose}>
                Train With Us
              </Link>
          </MenuItem>
          <Divider />
